...
var makeAsyncDriver = function makeAsyncDriver(options) {
  ...
  return function (request$, runStreamAdapter) {
    ...
    var _runStreamAdapter$mak = runStreamAdapter.makeSubject();

    var observer = _runStreamAdapter$mak.observer;
    var stream = _runStreamAdapter$mak.stream;

    var response$$ = runStreamAdapter.remember(stream);
